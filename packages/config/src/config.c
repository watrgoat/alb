...
static int parse_mac(const char *str, struct rte_ether_addr *addr)
{
	...
	if (sscanf(str, "%x:%x:%x:%x:%x:%x", &bytes[0], &bytes[1], &bytes[2],
		...
		if (event.type == YAML_SCALAR_EVENT) {
			...
			if (strcmp(value, "backends") == 0) {
				...
				if (expect_value) {
					if (strcmp(current_key, "ip") == 0) {
						...
						if (inet_aton(value, &addr))
							current_backend->ip =
							  ...
							  "port") == 0) {
						current_backend->port = htons(
						    (uint16_t)atoi(value));
					} else if (strcmp(current_key, "mac") ==
						   0) {
						parse_mac(
						    value,
						    &current_backend->mac);
					...
					strncpy(current_key, value,
						sizeof(current_key) - 1);
					current_key[sizeof(current_key) - 1] =
					    '\0';
					expect_value = 1;
				}
			}
