...
private:
    ...
    static constexpr uint32_t kWeights[3] = {16665000, 8635000, 16700000};

public:
    WaterFillingStrategy(ServerState *servers_, int count) 
        ...
        for (int i = 0; i < server_count && i < 3; ++i) {
            cumulative_weights[i] = (i == 0) ? kWeights[0] : cumulative_weights[i-1] + kWeights[i];
        }
    }
