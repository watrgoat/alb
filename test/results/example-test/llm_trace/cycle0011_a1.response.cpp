...
private:
    ...
    static constexpr uint32_t kWeights[3] = {12800000, 8335000, 16665000};
...
public:
    WaterFillingStrategy(ServerState *s, int count) 
        ...
        if (server_count > 1) cumulative_weights[1] = cumulative_weights[0] + kWeights[1];
        if (server_count > 2) cumulative_weights[2] = cumulative_weights[1] + kWeights[2];
    }
