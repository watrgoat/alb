...
class PacketReceiver:
    ...
    def add_packet(self, dst_ip, dst_port, dst_mac):
        with self.lock:
            self.packets.append(
                {"dst_ip": dst_ip, "dst_port": dst_port, "dst_mac": dst_mac}
            )
            ...
            try:
                ...
                if eth_type == 0x0800:  # IPv4
                    ...
                    if protocol == 17:  # UDP
                        ...
                        print(
                            f"[RX #{count}] {src_ip}:{src_port} -> {dst_ip}:{dst_port} "
                            f"(dst_mac={eth_dst})"
                        )
            except TimeoutError:
                continue
...
def sender(iface, dst_ip, dst_port, count, interval):
    ...
    try:
        ...
        # Get interface MAC
        info = fcntl.ioctl(
            sock.fileno(), 0x8927, struct.pack("256s", iface.encode()[:15])
        )
        src_mac = info[18:24]
        ...
        for i in range(count):
            ...
            eth_header = (
                b"\xff\xff\xff\xff\xff\xff" + src_mac + struct.pack("!H", 0x0800)
            )
...
def verify_round_robin(packets, expected_backends):
    ...
    if expected_backends:
        ...
        for i, ip in enumerate(dst_ips):
            ...
            if ip != expected_ip:
                print(
                    f"[VERIFY] FAIL: Packet {i} went to {ip}, expected {expected_ip} (round-robin)"
                )
                return False
...
def main():
    parser = argparse.ArgumentParser(
        description="UDP packet sender/receiver for ALB testing"
    )
    parser.add_argument(
        "--tx-iface", default="dtap0", help="Interface to send packets (default: dtap0)"
    )
    parser.add_argument(
        "--rx-iface",
        default="dtap1",
        help="Interface to receive packets (default: dtap1)",
    )
    parser.add_argument(
        "--dst-ip", default="192.168.1.1", help="Destination IP (default: 192.168.1.1)"
    )
    parser.add_argument(
        "--dst-port", type=int, default=5678, help="Destination port (default: 5678)"
    )
    parser.add_argument(
        "--count", "-c", type=int, default=5, help="Number of packets (default: 5)"
    )
    parser.add_argument(
        "--interval",
        "-i",
    ...
    parser.add_argument(
        "--rx-only", action="store_true", help="Only receive, do not send"
    )
    parser.add_argument(
        "--tx-only", action="store_true", help="Only send, do not receive"
    )
    parser.add_argument(
        "--verify-round-robin",
        action="store_true",
        help="Verify round-robin distribution",
    )
    parser.add_argument(
        "--expected-backends",
        nargs="+",
        help="Expected backend IPs in round-robin order",
    )
    ...
    if not args.tx_only:
        rx_thread = threading.Thread(
            target=receiver_thread, args=(args.rx_iface, stop_event)
        )
        rx_thread.start()
